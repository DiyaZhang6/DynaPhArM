...
def get_atom_scalar_features(rdkit_atom, config):
    """Calculates scalar features for a single RDKit atom based on config."""
    features = []
    cfg = config['drug_graph_task']
...
def get_bond_scalar_features(rdkit_bond, conformer, config):
...
def get_geometric_features(rdkit_mol, logger):
    ...
    try:
        ...
        for i in range(rdkit_mol.GetNumAtoms()):
            ...
            if len(neighbors) >= 2:
                # Create all combinations of 2 neighbors for the central atom i
                import itertools
                for j, k in itertools.combinations(neighbors, 2):
                    angle_rad = rdMolTransforms.GetAngleRad(conformer, j, i, k)
                    # Store as triplet of indices and the angle value
        ...
        for bond in rdkit_mol.GetBonds():
            ...
            # Find neighbors for atoms i and l to form the dihedral i-j-k-l
            if atom_j.GetDegree() > 1 and atom_k.GetDegree() > 1:
                for neighbor_of_j in atom_j.GetNeighbors():
                    i = neighbor_of_j.GetIdx()
                    if i == k: continue  # Avoid i and k being the same atom

                    for neighbor_of_k in atom_k.GetNeighbors():
                        l = neighbor_of_k.GetIdx()
                        if l == j: continue  # Avoid l and j being the same atom

                        # Ensure all four atoms are unique
                        if len({i, j, k, l}) == 4:
                            dihedral_rad = rdMolTransforms.GetDihedralRad(conformer, i, j, k, l)
                            # Store as quartet of indices and the angle value
...
def create_drug_graph_from_rdkit_mol(rdkit_mol, pdb_id, config, logger):
    ...
    node_features = [get_atom_scalar_features(atom, config) for atom in rdkit_mol.GetAtoms()]
    ...
    for rdkit_bond in rdkit_mol.GetBonds():
        ...
        bond_feat = get_bond_scalar_features(rdkit_bond, rdkit_mol.GetConformer(), config)
        edge_features.extend([bond_feat, bond_feat])

    edge_index_np = np.array(edge_indices, dtype=np.int64).T if edge_indices else np.empty((2, 0), dtype=np.int64)
    edge_features_np = np.array(edge_features, dtype=np.float32) if edge_features else np.empty(
        (0, len(config['drug_graph_task']['bond_types']) + 2), dtype=np.float32)
    ...
    graph_data = {
        ...
        'edge_scalar_features': torch.from_numpy(edge_features_np),
        'atom_coordinates': torch.from_numpy(geometry['atom_coordinates']),
        # Storing angle and dihedral data as lists of tuples (indices, value)
    ...
    logger.info(
        ...
        f"{rdkit_mol.GetNumBonds()} bonds, {len(graph_data['bond_angles'])} angles, "
        f"{len(graph_data['dihedral_angles'])} dihedrals."
    )
    return graph_data
...
if __name__ == "__main__":
    ...
    task_config = config['drug_graph_task']
    ...
    for folder in tqdm(pdb_id_folders, desc="Generating Drug Graphs"):
        ...
        try:
            ...
            suppl = Chem.SDMolSupplier(str(input_ligand_path), removeHs=False, sanitize=True)
            mol = next(suppl, None)
            ...
            drug_graph = create_drug_graph_from_rdkit_mol(mol, pdb_id, config, logger)
