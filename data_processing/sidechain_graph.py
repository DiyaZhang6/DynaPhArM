...
def create_sidechain_graphs_for_pdb(pdb_id: str, config: dict):
    ...
    task_config = config['sidechain_graph_task']
    ...
    try:
        ...
        for residue in sidechain_struct.get_residues():
            ...
            # Store atom names in the same order as they are processed
            ...
            graph_data = {
                # Save the identifiers needed for coordinate mapping
                'residue_id': res_id_tuple,  # e.g., ('A', 10, ' ')
                'atom_names': atom_names_in_order,  # e.g., ['CG', 'OD1', 'OD2']
                ...
                'edge_index': torch.tensor(np.array(edge_idx).T, dtype=torch.long) if edge_idx else torch.empty((2, 0),
                                                                                                                dtype=torch.long),
                'edge_s': torch.from_numpy(np.array(edge_s_list, dtype=np.float32)) if edge_s_list else torch.empty(
                    (0, len(task_config['bond_types']) + 3), dtype=torch.float32),
...
def main():
    ...
    task_config = CONFIG.get('sidechain_graph_task', {})
    if not task_config:
